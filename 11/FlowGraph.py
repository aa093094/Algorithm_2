...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        teamNums = len(self.teams)
        for i in range(teamNums):
            if i != teamId:
                if maxWin < self.wins[i]:
                    resultList.append(self.teams[i])

        if len(resultList) > 0:
            return True, resultList
        
        teamVertexNums = teamNums - 1
        gameVertexNums = int(teamVertexNums * (teamVertexNums - 1) / 2)
        vertexNumbers = int(teamVertexNums + gameVertexNums + 2)
        graph = FlowNetwork(vertexNumbers)
        copyTeams = self.teams.copy()
        copyTeams.pop(teamId)
        s = 1
        for i in range(len(self.teams)):
            if i != teamId:
                for j in range(i + 1, len(self.teams)):
                    if j != teamId:
                        graph.addEdge(FlowEdge(0, s, self.against[i][j]))
                        s += 1

        startVertex = 1
        r = gameVertexNums
        for i in range(0, teamNums - 1):
            for j in range(i + 1, teamNums - 1):
                if i != j:
                    graph.addEdge(FlowEdge(startVertex, startVertex + r + i, float('inf')))
                    graph.addEdge(FlowEdge(startVertex, startVertex + r + j, float('inf')))
                    startVertex += 1
                    r -= 1
        
        g = 0
        for t in range(gameVertexNums + 1, vertexNumbers - 1):
            versusTeamId = self.team2id[copyTeams[g]]
            graph.addEdge(FlowEdge(t, vertexNumbers - 1, maxWin - self.wins[versusTeamId]))
            g += 1

        fordAl = FordFulkerson(graph, 0, graph.V - 1)

        p = 0
        for i in range(gameVertexNums + 1, vertexNumbers - 1):
            if fordAl.inCut(i):
                targetTeam = copyTeams[p]
                resultList.append(targetTeam)
                p += 1
