...
def sap(g, aList, bList):    
    q = Queue()
    common_elements = [item for item in aList if item in bList]
    if common_elements:
    ...
    while not q.empty():
        ...
        for w in adj_nodes:
            if (w, side) not in visited:
                visited[(w, side)] = True
                distance[(w, side)] = v[2] + 1
                q.put((w, side, v[2] + 1))

            if (w, opposite_side) in visited:
                newsL = distance[(w, opposite_side)] + v[2] + 1
                if sapLength > newsL:
                    sca = w
                    sapLength = newsL 
