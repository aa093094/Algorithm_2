...
class SeamCarver:
    ...
    def findVerticalSeam(self):
        ...
        distTo = [[self.MAX_ENERGY] * graphWidth for _ in range(graphHeight)]
        edgeTo = [[None] * graphWidth for _ in range(graphHeight)]

        for y in range(1, graphHeight):
            for x in range(graphWidth):
                minDist = distTo[y-1][x]
                minEdge = x
                if x == 0:
                    if minDist > distTo[y-1][x+1]:
        ...
        resultList = [0] * self.height()
        minSeam = distTo[graphHeight - 1][0]
        minX = 0
        for x in range(1, graphWidth):
            if minSeam > distTo[graphHeight - 1][x]:
                minSeam = distTo[graphHeight - 1][x]
                minX = x

        for y in range(graphHeight - 1, -1, -1):
            resultList[y] = minX
            minX = edgeTo[y][minX]
