...
def sweepLine(segments):
    ...
    for segment in segments:
        if segment.isHorizontal() == True:
            pq.put((segment.x1, segment, start))
            pq.put((segment.x2, segment, end))
        else:
            pq.put((segment.x1, segment, end))
    
    bst = LLRB()
    result_list = []
    while pq.qsize() > 0:
        point = pq.get()
        if point[1].isHorizontal() == True and point[2] == start:
            bst.put(point[1].y1, point[1])
        elif point[1].isHorizontal() == True and point[2] == end:
            bst.delete(point[1].y1)
        elif point[1].isVertical() == True:
            inter_list = bst.rangeSearch(point[1].y1, point[1].y2)
            for inter in inter_list:
                result_list.append((bst.get(inter), point[1]))
